...
class Optimizer():
    ...
    def batch_optimization(self, model, X: np.ndarray, y: np.ndarray, 
                            n_cycles: int = 100000, learning_rate: float = 0.1, 
                            learning_rate_decay: bool = False):
        self._generate_params(model.params)
        for self.epoch in range(n_cycles):
            results = model.predict(X)
            gradients = model.gradient(X, y, results)
            model.params = self.update(model.params, gradients, learning_rate, learning_rate_decay)
        return model
    ...
    def minibatch_optimization(self, model, X: np.ndarray, y: np.ndarray, 
        ...
        for self.epoch in range(n_cycles):
            for t in range(m // self.batch_size):
                ...
                results = model.predict(mini_batch_X)
                gradients = model.gradient(mini_batch_X, mini_batch_y, results)
                model.params = self.update(model.params, gradients, learning_rate, learning_rate_decay)
